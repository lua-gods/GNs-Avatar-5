function genDocs(query)
		...
		if mline:find("^%-%-%-@class") then
			...
			for o = 1, 1000, 1 do
				...
				if line:find("^%-%-%-@field") then -- FIELD
					...
					local name, type, comment = (line .. "#"):match(
					"^%-%-%-@field[%s]+([%w_]+)[%s]+([^#]+)([%S%s]*)")
					if comment then
					...
					class_data.fields[#class_data.fields + 1] = { type = type, name = name, description = #
					comment ~= 0 and comment or "...", special = special }
			...
			table.sort(class_data.fields,
				function(a, b) return string.upper(a.name or "") < string.upper(b.name or "") end)
			...
			for c = 1, 1000, 1 do
				...
				elseif line:find("^%-%-%-@overload") then -- OVERLOAD
					...
					if line:match(":[%s]([%S])[%s]*$") then
						parameters_string, return_string = line:match(
						"^%-%-%-@overload fun(%([%s%S]*%)):%f[ ]([%s%S]*)")
					else
					...
					for param in string.gmatch(string.sub(parameters_string, 2, -2) .. ",", "[%s]*([%w_.: ]+),") do
						...
						if name ~= "self" then
							parameters[#parameters + 1] = { type = (#type ~= 0 and type or "any"), name =
							name }
						end
					end
	...
	for class_name, class_data in pairs(everything) do
		...
		if #class_data.fields > 0 then
			...
			for _, field in pairs(class_data.fields) do
				if field.type:find("GN.Event") then
					...
					"`|" ..
					(field.name or "") ..
					"|" .. field.description:gsub("|", "｜") .. "|" .. (field.special or " ") .. "|\n"
				end
			end
			if has_events then
				bake = bake .. "# Events\n"
				...
				for _, field in pairs(class_data.fields) do
					if field.type:find("GN.Event") then
						...
						"|`" ..
						field.name ..
						"`|" .. (field.description and field.description:gsub("|", "｜") or "...") .. "|\n"
					end
				end
			end
		end
		...
		for _, method in pairs(class_data.methods) do
			for _, func in pairs(method.overloads) do
				...
				for key, value in pairs(func.parameters) do
					...
					value.name ..
					" : " ..
					(value.type and value.type:gsub("|", "｜") or "") ..
					(key ~= #func.parameters and ", " or "")
				end
		...
		buffer:writeString(bake)
		buffer:setPosition(0)
		buffer:writeToStream(write)
